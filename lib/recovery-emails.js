...
// stage 1 at 1h (also clears any payment-settlement lag so we never email a
// customer who actually paid), stage 2 at 24h, stage 3 at 72h.
const STAGES = { 1: 1, 2: 24, 3: 72 };
...
function copyForStage(stage, order) {
  ...
  switch (stage) {
    ...
    default:
      return {
        subject: 'Last reminder — closing your saved basket',
        badge:   'Final reminder',
        heading: 'Before I clear your saved basket',
        intro:   `Hi ${fn}, this is the last reminder I'll send about this one &mdash; after today I'll clear the saved basket so stock frees up for other researchers. If now isn't the right time, no problem at all; everything stays available to order whenever your project needs it.`,
        // ── OPTIONAL INCENTIVE ──────────────────────────────────────────────
        // Declan: to add a thank-you on the final email only, replace the
        // empty string below with ONE of:
        //   '<p style="...">Finish today and I\'ll add bonus loyalty points to your account as a thank-you.</p>'
        //   '<p style="...">Use code WELCOMEBACK for free tracked shipping on this order.</p>'
        //   '<p style="...">Use code SAVE10 for 10% off if you complete within 24 hours.</p>'
        // Never put an incentive on stages 1 or 2 — it trains people to abandon.
        extra:   '',
      };
  }
}
...
function buildRecoveryEmail(stage, order, links) {
  const c = copyForStage(stage, order);
