...
async function proposeAction(o) {
  ...
  if (o.notify !== false) {
    const msg = title;
    try { if (sendPush) await sendPush({ title: 'Approval needed', body: msg, url: `${SITE}/admin/#actions` }); } catch (e) {}
    try { if (sendWhatsApp) await sendWhatsApp(`🔔 Approval needed: ${msg}\nReview: ${SITE}/admin/#actions`); } catch (e) {}
  }
