...
async function saveTokens({ access_token, refresh_token, expires_in }) {
  ...
  _tokCache = null; // force a fresh read after a refresh
}
...
// Token cache, per warm function instance. Every gapi() call used to re-read
// integration_tokens, so a single inbox sweep fired 25+ identical selects in a
// few seconds and helped saturate PostgREST. The row only changes on refresh
// (~hourly), so cache it and let saveTokens invalidate.
let _tokCache = null;          // { row, readAt }
let _tokInflight = null;       // de-dupes concurrent reads
const TOK_TTL_MS = 60 * 1000;

async function readTokenRow() {
  if (_tokCache && Date.now() - _tokCache.readAt < TOK_TTL_MS) return _tokCache.row;
  if (_tokInflight) return _tokInflight;
  _tokInflight = (async () => {
    try {
      const rows = await sbGet('integration_tokens?provider=eq.gmail&select=*&limit=1');
      const row = Array.isArray(rows) ? rows[0] : null;
      _tokCache = { row, readAt: Date.now() };
      return row;
...
async function getAccessToken() {
  let tok = await readTokenRow();
  if (!tok || !tok.refresh_token) {
    // Could be a stale cached miss (e.g. just reconnected) — re-read once.
    _tokCache = null;
    tok = await readTokenRow();
  }
...
async function connected() {
  try { const tok = await readTokenRow(); return !!(tok && tok.refresh_token); }
  catch { return false; }
}
