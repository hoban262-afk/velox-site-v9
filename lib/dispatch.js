...
async function decrementStock(items) {
  for (const it of items) {
    ...
    // Retatrutide 20mg is fulfilled as 2× 10mg — decrement 10mg by qty*2
    const is20mg = slug === 'retatrutide' && size.toLowerCase().replace(/\s/g, '') === '20mg';
    const effectiveSize = is20mg ? '10mg' : size;
    const effectiveQty  = is20mg ? qty * 2 : qty;
