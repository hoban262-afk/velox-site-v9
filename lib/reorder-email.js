...
function buildReorderEmail(order, links) {
  const fn = firstName(order);
  const prod = productNames(order.items);
  const rows = itemRows(order.items);
  const reorderUrl = links.reorderUrl;
  const unsubscribeUrl = links.unsubscribeUrl;

  const subject = `Running low on ${prod}? Reorder in a tap`;

  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Reorder your research peptides</title></head>
<body style="${S.body}">
...
<table width="600" cellpadding="0" cellspacing="0" border="0" style="${S.card}">
  ...
  <tr><td align="center" style="padding:0 40px 22px">
    <p style="margin:0;font-size:15px;color:#888;line-height:1.7">
      Hi ${fn}, it's been about a month since your last order with us. If your supply of ${prod} is running low, I've made it easy to reload your previous order into your basket &mdash; one tap and you're at checkout.
    </p>
  </td></tr>
