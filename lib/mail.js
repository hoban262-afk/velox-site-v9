...
// Marketing mail sends from news@veloxpeps.com (the already-verified root domain),
// kept distinct from order receipts. Override with MARKETING_FROM if needed.
const MARKETING_FROM = process.env.MARKETING_FROM || 'Velox Peptides <news@veloxpeps.com>';
...
// ── Suppression backstop ("do not email me") ─────────────────────────────────
// email_suppressions is keyed by address alone, so it covers guest buyers who
// never existed in `subscribers` / `newsletter_codes` — the gap that made the
// unsubscribe link a no-op for roughly a third of marketing recipients.
//
// Each sequence still does its own pre-check (so it can park the record and
// stop re-checking). This is the last line of defence: every marketing send
// funnels through sendMail, so one sequence forgetting cannot leak an email.
//
// Exempt flows are transactional — an affiliate must still be told they have
// been paid, and admin test sends must always work. Order receipts and
// dispatch notifications never reach here at all; they call Resend directly.
const NEVER_SUPPRESS = new Set(['affiliate-commission', 'affiliate-payout', 'test']);

async function isSuppressed(email) {
  const r = await fetch(
    `${SUPABASE_URL}/rest/v1/email_suppressions?email=eq.${encodeURIComponent(email)}&select=email&limit=1`,
    { headers: sbHeaders },
  );
  if (!r.ok) throw new Error(`suppression lookup -> ${r.status}`);
 ...
 * @returns {Promise<{id:string|null, ok:boolean, suppressed?:true, deferred?:true}>}
 *   suppressed — recipient has opted out; do not retry.
 *   deferred   — could not verify opt-out status; safe to retry next run.
 */
async function sendMail(o) {
  const to = String(o.to || '').toLowerCase().trim();
  if (!to) return { id: null, ok: false };

  // Checked before logMessage so a blocked send never appears in analytics as
  // though it went out. Fails CLOSED: if we cannot prove the address is safe to
  // email we refuse. Supabase 504s are a known intermittent here, and ok:false
  // leaves the caller's stage unadvanced, so it simply retries on the next run.
  if (!NEVER_SUPPRESS.has(o.flow)) {
    try {
      if (await isSuppressed(to)) {
        console.log(`[mail] suppressed ${o.flow || 'campaign'} -> ${to}`);
        return { id: null, ok: false, suppressed: true };
      }
    } catch (e) {
      console.error(`[mail] suppression check failed, refusing to send ${o.flow || 'campaign'}:`, e.message);
      return { id: null, ok: false, deferred: true };
    }
  }
  ...
  // Resend's default rate limit is ~2 req/s; on a burst (campaigns) some sends
  // come back 429. Retry those a few times with backoff so they don't silently
  // fail. Non-rate-limit errors fail fast.
  const isRateLimited = (err) =>
    err && (err.statusCode === 429 || err.name === 'rate_limit_exceeded' ||
      /rate.?limit|too many requests/i.test(err.message || err.error || ''));
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      const resp = await resend.emails.send(payload);
      if (resp && resp.error) {
        if (isRateLimited(resp.error) && attempt < 3) { await sleep(600 * (attempt + 1)); continue; }
        console.error('[mail] resend error', resp.error); return { id, ok: false };
      }
      if (resp && resp.data && resp.data.id) await setProviderId(id, resp.data.id);
      return { id, ok: true };
    } catch (e) {
      if (isRateLimited(e) && attempt < 3) { await sleep(600 * (attempt + 1)); continue; }
