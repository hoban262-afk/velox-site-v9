...
async function getOrdersInfo(identifiers) {
  if (!API_KEY) return [];
  const ids = (Array.isArray(identifiers) ? identifiers : [identifiers]).filter(Boolean).join(',');
  if (!ids) return [];
  const r = await cdFetch(`/api/v1/orders/${encodeURIComponent(ids)}`);
  if (!r.ok) return [];
...
module.exports = {
  ...
  getOrdersInfo, buildOrderPayload, countryToIso, sbGet, sbWrite, API_BASE,
};
