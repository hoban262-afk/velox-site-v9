 ...
 *                                     Omit to let Click & Drop pick at print time.
 *   CLICKANDDROP_INTL_SERVICE_CODE  — service code for non-GB orders.
 *   CLICKANDDROP_PACKAGE_FORMAT     — default "smallParcel".
 *   CLICKANDDROP_UNIT_WEIGHT_G      — per-item weight in grams, default 100.
...
const PACKAGE_FORMAT = (process.env.CLICKANDDROP_PACKAGE_FORMAT || 'smallParcel').trim();
const UNIT_WEIGHT_G  = Math.max(1, parseInt(process.env.CLICKANDDROP_UNIT_WEIGHT_G || '100', 10) || 100);
...
// ── Country name → ISO-2 (covers the UK + the EU dropdown in checkout) ──────
const ISO = {
...
function buildOrderPayload(order) {
  ...
  const contents = items.map((it) => ({
    ...
    SKU: it.slug ? String(it.slug).slice(0, 100) : undefined,
    quantity: num(it.qty || it.quantity) || 1,
    unitValue: Number(num(it.price).toFixed(2)),
    unitWeightInGrams: UNIT_WEIGHT_G,
  ...
  return {
    ...
    packages: [{
      weightInGrams: Math.min(30000, Math.max(1, UNIT_WEIGHT_G * totalQty)),
...
module.exports = {
  ...
  buildOrderPayload, countryToIso, sbGet, sbWrite, API_BASE,
};
